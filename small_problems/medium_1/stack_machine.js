...
input: string
output: integer, or nothing, depending on the string input;

requirements:
  -parse the string argument to discover a list of commands;
  -match these commands with their appropriate action, as defined above
  -perform the tasks in the action
  -for commands DIV and MOD, make sure the output is an integer.
  -the basic structure is a stack and register. The stack is a list of values, and the register is a single standalone value. Operations may involve the register or the stack or both. See the definitions above.
  -assume valid input, no error handling

data structure/algorithm:
  -initialize stack and register variables assigned to [] and 0, respectively
  -create an array of values, equal to the input string split by a space.
  -loop over the array:
    -use a switch statement to evaluate each value;
    -perform the action as defined by that value

more formally,
-DEFINE FUNCTION minilang(commands)
  -DEFINE stack = []
  -DEFINE register = 0
  -DEFINE arrOfCommands = commands.split(' ')
  -FOR (let idx = 0; idx < arrOfCommands.length; idx += 1) {
    switch(arrOfCommands[idx]) {
      case 'PUSH'
      break
      case 'ADD'
      break
...
function minilang(commands) {
  let stack = [];
  let register = 0;
  let arrOfCommands = commands.split(' ');

  for (let idx = 0; idx < arrOfCommands.length; idx += 1) {
    let command = arrOfCommands[idx];
    register = executeCommand(command, register, stack);
  }

}

function executeCommand(command, register, stack) {
  switch(command) {
    case 'PUSH':
      stack.push(register);
      break;
